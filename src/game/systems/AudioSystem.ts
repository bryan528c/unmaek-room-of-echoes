...
export type SoundName = 'slash' | 'hit' | 'dash' | 'warning' | 'parryOpen' | 'parry' | 'perfectParry' | 'target' | 'sentenceFull' | 'empowerReady' | 'stop' | 'rewind' | 'link' | 'chain' | 'chainStop' | 'chainBackflow' | 'chainRegression' | 'bossVulnerable' | 'upgrade' | 'hurt' | 'critical' | 'phase' | 'victory' | 'defeat';
...
const NOTES: Record<SoundName, readonly [number, number, OscillatorType, number]> = {
  ...
  perfectParry: [920, 1560, 'sine', 0.18], target: [410, 520, 'sine', 0.055], empowerReady: [480, 860, 'triangle', 0.22],
  ...
  chainStop: [280, 920, 'triangle', 0.26], chainBackflow: [880, 240, 'sine', 0.3], chainRegression: [520, 170, 'triangle', 0.34], bossVulnerable: [210, 740, 'sine', 0.25],
